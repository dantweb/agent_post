...
def test_message_repository_save_and_retrieve(message_repo):
...
def test_message_repository_delete(message_repo):
    msg = Message(id=1, created_at=datetime.now(), collected_at=datetime.now(), from_address="from", to_address="to",
                  data="data")
    message_repo.save(msg)
...
def test_message_repository_find_by_address(message_repo):
